...
const pullRequestBody = "再publishを行うためにラベルのみ付けたPullRequestです。何も変更はありません。";
...
try {
	...
	// PRを作成する
	const pullReqDataString = execSync(`curl --fail -H "Authorization: token ${process.env.GITHUB_AUTH}" -X POST -d '{"title":"${pullRequestTitle}", "body":"${pullRequestBody}", "head":"dera-:${branchName}", "base":"master"}' ${apiBaseUrl}/pulls`).toString();
