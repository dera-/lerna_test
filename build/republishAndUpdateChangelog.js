...
const pullReqDataString = execSync(`curl --fail -H "Authorization: token ${process.env.GITHUB_AUTH}" -X POST -d '{"title":"v${currentVersion}", "body":"※自動作成されたPRです", "head":"dera-:${branchName}", "base":"master"}' https://api.github.com/repos/dera-/lerna_test/pulls`).toString();
const pullReqData = JSON.parse(pullReqDataString);
// issue(PR)にラベル付ける
execSync(`curl --fail -H "Authorization: token ${process.env.GITHUB_AUTH}" -X POST -d '{"labels": ["republish"]}' https://api.github.com/repos/dera-/lerna_test/issues/${pullReqData["number"]}/labels`);
// PRのマージ
