...
const currentChangeLog = fs.readFileSync(path.join(__dirname, "..", "CHANGELOG.md"));
const addedLog = execSync(`${path.join(__dirname, "..", "node_modules", ".bin", "lerna-changelog")}`).replace("Unreleased", nextVersion);
const nextChangeLog = currentChangeLog.replace("# CHANGELOG\n\n", "# CHANGELOG\n\n" + addedLog);
fs.writeSync(path.join(__dirname, "..", "CHANGELOG.md"), nextChangeLog);
